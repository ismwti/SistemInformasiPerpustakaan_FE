...
<script lang="ts">
    ...
    async function muatKategori() {
        ...
        try {
            ...
            if (!respon.ok) throw new Error('Gagal sync data kategori dengan server.');
    ...
    async function simpanKategori(e: Event) {
        ...
        const pesanKonfirmasi = idKategoriTerpilih 
            ? 'Yakin ingin menyimpan perubahan pada kategori ini?' 
            : 'Yakin ingin menambahkan kategori baru ini?';
    ...
    async function hapusKategori(id: number) {
        if (!confirm('Serius mau delete kategori ini? Relasi buku mungkin akan terpengaruh.')) return;
...
<div class="app-layout">
    ...
    <main class="main-content">
        <header class="app-header">
            <div class="brand">
                ...
                <p>Kelola seluruh kelompok genre pustaka sistem perpustakaan secara real-time.</p>
            </div>
        ...
        <div class="card-table-wrapper">
            <div class="card-header">
                <h2>Daftar Kategori Tersimpan</h2>
                <span class="count-badge">{daftarKategori.length} Items</span>
            </div>
            ...
            {:else if daftarKategori.length === 0}
                <div class="alert-zone empty">
                    <p>Kategori kosong melompong. Klik tombol di atas buat nambahin.</p>
                </div>
            {:else}
                <div class="table-scroll">
                    <table>
                        ...
                        <tbody>
                            {#each daftarKategori as item}
                                <tr>
                                    <td class="secondary-text"><code>#{item.id}</code></td>
                                    <td class="primary-text">{item.namaKategori || item.nama_kategori}</td>
                                    <td>
                                        <div class="action-flex">
                                            <button class="action-btn edit" onclick={() => editKategori(item)} title="Edit">
                                            ...
                                            <button class="action-btn delete" onclick={() => hapusKategori(item.id)} title="Delete">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="action-icon">
    ...
    <!-- SIDE DRAWER MODAL FORM (LIGHT MECHANISM) -->
    {#if isFormOpen}
        <div class="drawer-overlay" onclick={() => isFormOpen = false} role="presentation"></div>
        <div class="drawer-container">
            <div class="drawer-header">
                <h3>{idKategoriTerpilih ? 'Update Kategori' : 'Tambah Kategori Baru'}</h3>
                <button class="close-drawer" onclick={() => isFormOpen = false}>×</button>
            </div>

            <form onsubmit={simpanKategori} class="drawer-form">
                <div class="group-field">
                    <label for="namaKategori">NAMA KATEGORI</label>
                    <input type="text" id="namaKategori" bind:value={namaKategori} required placeholder="e.g. Fiksi, Sains, Komputer" />
                </div>
                ...
                <div class="drawer-actions">
                    <button type="submit" class="btn-submit-form">
                        ...
                        {idKategoriTerpilih ? 'Simpan Perubahan' : 'Publish Kategori'}
                    </button>
                    {#if idKategoriTerpilih}
